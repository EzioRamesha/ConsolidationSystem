...
namespace ConsolidationSystem.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        [HttpPost]
        public ActionResult Qnetlnv(HttpPostedFileBase Compfile, HttpPostedFileBase CRMfiles)
        {
            if (ModelState.IsValid)
            {
                ViewBag.Message = Methods.ConsolidationProcess(Compfile, CRMfiles, "Qnetlnv", User.Identity.Name);
                #region for now
                ...
                //    _HistoryViewModel.Type = "Qnetlnv";
                ...
                //                        _ProcessResultInvView.AmountDiff = deciAmountDiff;
                //                    }
                //                    _ProcessResultInvView.Type = "Qnetlnv";
        ...
        [HttpPost]
        public ActionResult GetProcessResult(string strType)
        {
            var varReturn = Json(_ProcessResultBAL.GetAll(strType), JsonRequestBehavior.AllowGet);
            
            
            return Content(JsonConvert.SerializeObject(varReturn.Data));
        }

        [HttpPost]
        public ActionResult GetBatchNo(string strType)
        {
          
            var varReturn = Json(_ProcessResultBAL.GetBatchNo(strType), JsonRequestBehavior.AllowGet);
            return Content(JsonConvert.SerializeObject(varReturn.Data));
        }
        public ActionResult GetAllHistory()
        {
            var varReturn = Json(_HistoryBAL.GetAll(), JsonRequestBehavior.AllowGet);
            return Content(JsonConvert.SerializeObject(varReturn.Data));
        }
        

        [HttpPost]
        public ActionResult BatchDelete(string strBatchNo)
        {
            var varReturn = Json(_ProcessResultBAL.BatchDeleteProcessResult(strBatchNo), JsonRequestBehavior.AllowGet);
            return Content(JsonConvert.SerializeObject(varReturn.Data));
        }
        ...
        [HttpPost]
        public ActionResult GetImportDoc(string strType)
        {
            var varReturn = Json(_ImportDocBAL.GetAll(strType), JsonRequestBehavior.AllowGet);
            return Content(JsonConvert.SerializeObject(varReturn.Data));
        }
        public ActionResult ExportQnetlnv(string strType)
        {
            string fileName = "Qnetlnv-" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xlsx";

            var forms = _ProcessResultBAL.GetAll("Qnetlnv");
